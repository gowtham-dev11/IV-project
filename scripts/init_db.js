...
const supabaseUrl = process.env.VITE_SUPABASE_URL || 'https://nnihbqxzssgmzlpuutld.supabase.co';
const supabaseKey = process.env.VITE_SUPABASE_PUBLISHABLE_KEY || 'sb_publishable_GlczctFzltukwakvuw0MtA_2F6kkFgE';
...
async function main() {
  console.log('Testing Supabase Connection & Table Seeding...');
  
  // Test reading students
  const { data: students, error: readError } = await supabase.from('students').select('*');
  if (readError) {
    console.log('Students table read result:', readError.message);
  } else {
    console.log(`Current students count in DB: ${students ? students.length : 0}`);
  }

  // Attempt upserting 53 students
  const { data: upsertData, error: upsertError } = await supabase
    .from('students')
    .upsert(STUDENTS_DATA, { onConflict: 'roll_number' })
    .select();

  if (upsertError) {
    console.log('Upsert error:', upsertError.message);
  } else {
    console.log(`Successfully seeded/upserted ${upsertData.length} students into Supabase!`);
  }
}
